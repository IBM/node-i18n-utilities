...
require.extensions['.properties'] = function(module, filename) {
    ...
    return module._compile('module.exports = require("' + __dirname + '/i18n")._loadBundle("' + filename + '");', filename);
};
 ...
 */
exports._loadBundle = function(filename) {
    if (!resolveBundle(filename)) {
        ...
        var dir = path.dirname(filename),
            basename = path.basename(filename, FILE_EXTENSION),
...
function processBundle(filename) {
    ...
    var dir = path.dirname(filename),
        ...
        basepath = dir + '/' + matches[1] + FILE_EXTENSION,
        lang = matches[2],
        country = matches[3];
