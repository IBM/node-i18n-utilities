...
suite("i18n tests", function() {
    ...
    test("bundle locale function", function() {
        var test = require('./data/test.properties'),
            test2 = require('./data/test2.properties');

        // default locale is not specified in the root bundle; in our case it's english
        // but this isn't necessarily the case. So it returns undefined.
        assert.equal(test2.locale(), undefined);
        assert.equal(test2.locale('en'), undefined);
        assert.equal(test2.locale('fr'), "fr");
        assert.equal(test2.locale("fr-FR"), "fr");
        assert.equal(test2.locale("fr-CA"), "fr-CA");
        assert.equal(test.locale("ja"), undefined);
        assert.equal(test.locale("ja-JP"), undefined);

        assert.equal(test2.locale(makeRequest()), undefined);
        assert.equal(test2.locale(makeRequest('en')), undefined);
        assert.equal(test2.locale(makeRequest('fr')), "fr");
        assert.equal(test2.locale(makeRequest("fr-FR")), "fr");
        assert.equal(test2.locale(makeRequest("fr-CA")), "fr-CA");
        assert.equal(test.locale(makeRequest("ja")), undefined);
        assert.equal(test.locale(makeRequest("ja-JP")), undefined);
    });
